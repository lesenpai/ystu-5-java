...
public class LoginServ11 extends HttpServlet {
    ...
    public void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
        ...
        if (login(us, pass)) {
            if (i == 1)
                response.sendRedirect("http://localhost:4444/myApp/WelcomeServlet71");
            else
            if (i == 2)
                response.sendRedirect("http://localhost:4444/myApp/WelcomeServlet81");
            else
            if (i == 3)
                response.sendRedirect("http://localhost:4444/myApp/WelcomeServlet91");
            else
            if (i == 4) {
            ...
            response.sendRedirect("http://localhost:4444/jsp/vvod.jsp");
        }
    }
