...
public class LoginServlet extends HttpServlet {
	...
	private void sendLoginForm(HttpServletResponse response, String message) throws IOException {
		...
		// вход через cookie
		out.println("<tr>");
		out.println("<td><input type=submit name='loginUsingSession' value='Login (using session)'></td>");
		out.println("</tr>");
	...
	public void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		// вход через session
		else if (request.getParameter("loginUsingSession") != null) {
			// отправка session браузеру
			System.out.println("/ login using session");
			response.setContentType("text/html");
			PrintWriter out = response.getWriter();
			HttpSession session = request.getSession(true);
			session.setAttribute("loggedIn", "true");
			out.println("<META HTTP-EQUIV=Refresh CONTENT=0;URL=session>");
		}
